...
Проверять при генерации кода, если у селектора больше 1 результата, то добавлять .first() в путь
...
На 8м сайте уделить внимание поляь article и brand
    ...
    У brand - это тип - извлечение характеристик. Нужно писать отдельную функцию-обработчик
    и возможно указывать во входных данных, что на странице товара есть блок с характеристиками,
    и приводить примеры полей из него

Написать реализацию selectorChecker(), которая будет возвращать текст уже для финального шаблона кода

Нужно прогнать юнит-тестами все функции, которые я написал
И посмотреть их результаты, по отдельности




Позже:
Написать отдельный модуль по извлечению и парсингу JSON
...
Добавить использование прокси (чезе curl-ы)
Добавить traysBack того, как он генерировал этот парсер
Добавить бота в Тг
